...
class CollectCvsApiTest extends PHPUnit_Framework_TestCase
{
    ...
    public function test_create_order_register_transaction()
    {
        ...
        */
        ...
        $api = new CollectCvsApi($options, $httpClient, $messageFactory);
        $xml = $api->createRequestXML($order, 'cvs_order_regiater');
        ...
        */
        ...
        $this->assertContains($custOrderNumber, $xml);
        $this->assertContains($orderAmount, $xml);
        $this->assertContains($expireDate, $xml);
        $this->assertContains($payerName, $xml);
        $this->assertContains($payerPostcode, $xml);
        $this->assertContains($payerAddress, $xml);
        $this->assertContains($payerMobile, $xml);
        $this->assertContains($payerEmail, $xml);
    }
